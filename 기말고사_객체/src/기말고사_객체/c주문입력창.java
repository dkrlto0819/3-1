	   ...
	   public void actionPerformed(ActionEvent ae) {
		   switch(ae.getActionCommand()){
			   case "확 인" :
				  ...
				  주문수량들=new int[음식수+1];
					for (int i = 1; i <= 음식수; i++) 주문수량들[i] = Integer.parseInt(tx수량들[i].getText());
				
					제어자_객체.주문량_저장하기(주문수량들);
					제어자_객체.가격계산();
					int 합계=제어자_객체.총액_가져오기();
					int[] 최종계산=제어자_객체.최종계산_가져오기();
				  
				  Frame f=new Frame();
				  c주문결산창 출력창=new c주문결산창(f, 음식수);
				  출력창.자료_설정(음식명들, 주문수량들, 음식가격들, 최종계산, 합계);
			      출력창.화면_출력();
